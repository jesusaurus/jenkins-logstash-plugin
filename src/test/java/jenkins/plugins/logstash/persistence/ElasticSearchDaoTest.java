...
public class ElasticSearchDaoTest {
  ...
  public void before() throws Exception {
    ...
    dao = new ElasticSearchDao("http://localhost", port, "/jenkins/logstash", "username", "password");

    // Note that we can't run these tests in parallel
    dao.clientBuilder = mockClientBuilder;
  ...
  public void constructorFailNullHost() throws Exception {
    try {
      new ElasticSearchDao(null, 8200, "logstash", "username", "password");
  ...
  public void constructorFailEmptyHost() throws Exception {
    try {
      new ElasticSearchDao(" ", 8200, "logstash", "username", "password");
    } catch (IllegalArgumentException e) {
      assertEquals("Wrong error message was thrown", "host name is required", e.getMessage());
      throw e;
    }
  ...
  public void constructorFailMissingScheme() throws Exception {
    try {
      new ElasticSearchDao("localhost", 8200, "logstash", "username", "password");
    } catch (IllegalArgumentException e) {
  ...
  public void constructorFailNullKey() throws Exception {
    try {
      new ElasticSearchDao("http://localhost", 8200, null, "username", "password");
    } catch (IllegalArgumentException e) {
  ...
  public void constructorFailEmptyKey() throws Exception {
    try {
      new ElasticSearchDao("http://localhost", 8200, " ", "username", "password");
    } catch (IllegalArgumentException e) {
      assertEquals("Wrong error message was thrown", "elastic index name is required", e.getMessage());
      throw e;
    }
  ...
  public void constructorSuccess1() throws Exception {
    // Unit under test
    dao = new ElasticSearchDao("https://localhost", 8200, "logstash", "username", "password");
  ...
  public void constructorSuccess2() throws Exception {
    // Unit under test
    dao = new ElasticSearchDao("http://localhost", 8200, "jenkins/logstash", "", "password");
  ...
  public void constructorSuccess3() throws Exception {
    // Unit under test
    dao = new ElasticSearchDao("http://localhost", 8200, "/jenkins//logstash/", "userlongername", null);
  ...
  public void getPostSuccessNoAuth() throws Exception {
    String json = "{ 'foo': 'bar' }";
    dao = new ElasticSearchDao("http://localhost", 8200, "/jenkins/logstash", "", "");
    dao.clientBuilder = mockClientBuilder;
    ...
    assertEquals("Wrong content", json, stream.toString(CharEncoding.UTF_8));
  }

  @Test
  public void getPostSuccessAuth() throws Exception {
    String json = "{ 'foo': 'bar' }";
    dao = new ElasticSearchDao("https://localhost", 8200, "/jenkins/logstash", "username", "password");
    dao.clientBuilder = mockClientBuilder;
  ...
  public void pushFailStatusCode() throws Exception {
    String json = "{ 'foo': 'bar' }";
    dao = new ElasticSearchDao("http://localhost", 8200, "/jenkins/logstash", "username", "password");
    dao.clientBuilder = mockClientBuilder;
